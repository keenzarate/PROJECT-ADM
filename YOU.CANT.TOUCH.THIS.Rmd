...
```{r}
ggplot(attrition,aes(x=EducationField,y=MonthlyIncome,col=EducationField))+
  geom_jitter(size=2)+
  facet_grid(".~Gender")
```
...
ggplot(attrition,aes(x=WorkLifeBalance,y=MonthlyIncome,col=Department))+
  geom_jitter(size=2)+
...
ggplot(attrition,aes(x=BusinessTravel,fill=Attrition))+
  geom_bar()+labs(y="Count",title="Attrition vs Business Travel")+scale_fill_brewer(palette = "Set2")
```
...
**Logistic**
```{r}
mod.log <- glm(Attrition ~., family=binomial(link="logit"),data=train.df)
summary(mod.log)
```
